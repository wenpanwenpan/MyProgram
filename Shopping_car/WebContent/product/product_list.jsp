...
<c:forEach items="${all}" var="pro" >
	<tr onMouseOver="changeColor(this,'white')" onMouseOut = "changeColor(this,'F2F2F2')">
		<td>${pro.pid}</td>
		<td><a href = "${pageContext.request.contextPath }/ProductBackServlet?pid=${pro.pid}&status=show">${pro.name}</a></td>
		<td>${pro.price}</td>
		<td>${pro.amount}</td>
		<td>${pro.note}</td>	
		<td>${pro.count}</td>
		<td><a href = "${pageContext.request.contextPath }/ProductBackServlet?pid=${pro.pid}&status=add_cars" target="_ablank">增加到购物车</a></td>
	</tr>	
</c:forEach>
</table>
</center>
