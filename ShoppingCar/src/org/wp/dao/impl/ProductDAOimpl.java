...
public class ProductDAOimpl implements IProductDAO {
	...
	public boolean doCreate(Product vo) throws Exception {
		boolean flag = false;
		String sql = "insert into product1(pid,name,price,amount,note,photo) values(proseq1.nextval,?,?,?,?,?)";
		this.pstmt = this.conn.prepareStatement(sql);
		this.pstmt.setString(1, vo.getName());
		this.pstmt.setDouble(2, vo.getPrice());
		this.pstmt.setInt(3, vo.getAmount());
		this.pstmt.setString(4, vo.getNote());
		this.pstmt.setString(5, vo.getPhoto());
	...
	public boolean doUpdate(Product vo) throws Exception {
		boolean flag = false;
		String sql = "update product1 set name = ?,note = ?,price = ?,amount = ?,photo = ? where pid = ?";
		this.pstmt = this.conn.prepareStatement(sql);
		this.pstmt.setString(1, vo.getName());
