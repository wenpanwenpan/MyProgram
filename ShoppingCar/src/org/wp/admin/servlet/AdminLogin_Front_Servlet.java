...
public class AdminLogin_Front_Servlet extends HttpServlet{
	...
	String pages = "login/front_login.jsp";				//要跳转的页面
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(errors.size() == 0) {			//验证登录
			try {
				Member mem = new Member();		//登录的用户
				mem.setMid(mid);
				mem.setPassword(password);
				if(DAOFactory.getMemberDAOInstance().findLogin(mem)) {		//如果该用户存在于数据库 member 表
					request.getSession().setAttribute("admin", mem); 			//保存已经登录的member对象
